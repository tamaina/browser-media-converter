...
- Resizing automatically uses `webcodecs-color.resizeVideoFrame()`: supported planar YUV/YUVA and `NV12` frames use raw planar resize, while RGB, packed, unknown, or otherwise unsupported `VideoFrame` formats fall back to Canvas and produce an `RGBA` frame.
