...
- Resizing automatically uses `webcodecs-color.resizeVideoFrame()`: supported planar YUV/YUVA and `NV12` frames use CPU planar resize, while `RGBA`, `RGBX`, `BGRA`, `BGRX`, and unsupported or unknown `VideoFrame` formats resize through Canvas and return `RGBA` with a warning.
