...
var IsobmffBoxWriter = class {
  ...
  writeAscii(text2) {
    for (let i = 0; i < text2.length; i++) {
      this.helperView.setUint8(i % 8, text2.charCodeAt(i));
      if (i % 8 === 7)
        this.writer.write(this.helper);
    }
    if (text2.length % 8 !== 0) {
      this.writer.write(this.helper.subarray(0, text2.length % 8));
    }
  }
  writeBox(box2) {
...
var ascii = (text2, nullTerminated = false) => {
  const bytes2 = Array(text2.length).fill(null).map((_, i) => text2.charCodeAt(i));
  if (nullTerminated)
    bytes2.push(0);
  return bytes2;
};
...
// src/index.ts
async function mp4ToHls(options) {
  ...
  return [...assets.entries()].map(([path, asset]) => {
...
function createMpegTsHlsFormat(options = {}) {
...
function findMasterPlaylist(assets) {
